...
class Ebook extends MY_Controller {
	...
	public function ebook()
	{
		...
		if(!empty($cek) && $stts=='petugas')
		{
			...
			$tmp['content']=$this->load->view('petugas/ebook/R_ebook',$data, TRUE);
	...
	public function hapus_ebook()
	{
		...
		if(!empty($cek) && $stts=='petugas')
		{
			...
			header('location:'.base_url().'petugas/Ebook/ebook');
		}
		else
		{
			header('location:'.base_url().'web/log');
	...
	public function tambah_ebook()
	{
		...
		{	
			...
			if ($this->form_validation->run() === FALSE)
			{
				...
				/*masukan data kedalam view */
				$tmp['content']=$this->load->view('petugas/ebook/C_ebook',$data, TRUE);
				$this->load->view('petugas/layout',$tmp);
			}
			else
			{
	...
	public function edit_ebook()
	{
		...
		{	
			...
			/*layout*/	
				...
				/*masukan data kedalam view */
				$tmp['content']=$this->load->view('petugas/ebook/U_ebook',$data, TRUE);
				$this->load->view('petugas/layout',$tmp);		
		}
		else
		{
		...
		{	
			...
			else
			{
				...
				if ($upload_file) {
					...
					redirect('petugas/Ebook/ebook','refresh');
				} else {
					$data= array (
						'id_ebook' => $this->input->post('id_ebook'),
						'nama_ebook' => $this->input->post('nama_ebook'),
